...
export default function InstancePreviewModal({ instance, onClose }: InstancePreviewModalProps) {
  const [activeTab, setActiveTab] = useState(0)

  useEffect(() => {
    // Reset to metadata tab when instance changes
    if (instance) {
      setActiveTab(0)
    }
  }, [instance])
            ...
            <div className="space-y-4 font-hand">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm">{Math.round(metadata.size / 1024)} KB</div>
                </div>
                <div>
                  <div className="text-xs text-slate-500 uppercase">Fecha de Creación</div>
                  <div className="text-sm">{new Date(metadata.createdAt).toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-500 uppercase">Tipo de Problema</div>
                  <div className="text-sm uppercase">{metadata.problemType}</div>
                </div>
              </div>

              <div className="pt-4 border-t border-slate-200">
                <div className="text-sm font-bold uppercase mb-3">Dimensiones</div>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <div className="text-xs text-slate-500 uppercase">Jobs</div>
                    <div className="text-2xl font-bold">{metadata.jobs || '—'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-500 uppercase">Máquinas</div>
                    <div className="text-2xl font-bold">{metadata.machines || '—'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-500 uppercase">Operaciones</div>
                    <div className="text-2xl font-bold">{metadata.operations || '—'}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 font-hand">
              <div className={`p-4 rounded ${
                ...
                <div className={`font-bold uppercase ${
                  ...
                  {metadata.validated ? '✓ Instancia Válida' : '✗ Instancia con Errores'}
                </div>
              </div>

              {metadata.validationErrors && metadata.validationErrors.length > 0 && (
                <div>
                  <div className="font-bold uppercase text-red-700 mb-2">Errores de Validación:</div>
              ...
              {metadata.validated && (
                <div className="text-sm text-slate-600">
                  Esta instancia ha pasado todas las validaciones y está lista para ser ejecutada.
                </div>
              )}
            </div>
          )}
